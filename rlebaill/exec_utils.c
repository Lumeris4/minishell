...
/*   Updated: 2025/01/22 17:54:13 by rlebaill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_have_rights(char *path)
{
	...
	// {
	// 	close(fd[0]);
	// 	exit(ft_heredoc(splited_split[0], 1, fd[1]));
	// }
	// else if (have_right(splited_split[0]))
	// {
	// 	close(fd[1]);
	// 	exit(ft_redirection(splited_split[0], mini->envp, 1, fd[0]));
	// }
	...
	int prev_fd, t_mini *mini)
{
	if (splited_split)
	{
		...
		close(fd[0]);
		close(fd[1]);
		exit(ft_command(splited_split[0], splited_split, "ok", mini));
	}
}
